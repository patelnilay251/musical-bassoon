...
export function skyColor(S, dx, dy, dz, out, disc = true) {
  ...
  // The horizon's tone in the lowest few degrees, blue laid over it, and
  // a deep cobalt that takes over well before the top of the picture.
  const blue = 1 - Math.exp(-e / 0.07);
  let r = hr + (M[0] - hr) * blue;
  let g = hg + (M[1] - hg) * blue;
  let b = hb + (M[2] - hb) * blue;
  const deep = Math.pow(smoothstep(0.02, 0.55, e), 0.9);
  r += (Z[0] - r) * deep;
  g += (Z[1] - g) * deep;
  b += (Z[2] - b) * deep;
  ...
  const uneven = 1 + n * 0.015 * (0.35 + blue);
  r *= uneven;
  g *= uneven;
  b *= uneven;
  ...
  for (let i = 0; i < S.streaks.length; i++) {
    ...
    const q = u * u + v * v;
    if (q < 1) {
      // Only low suns light them; a clear day's sky has none.
      const a = 0.55 * smoothstep(1, 0.25, q) * smoothstep(12, 4, S.sunEl);
      const L = S.cloudLit;
